...
function handleKeyDown(e) {
    ...
    if (e.key == "ArrowUp" || e.key == "ArrowDown") {
        if (document.activeElement.id == "kopi") {
            e.preventDefault();
            if (e.key == "ArrowUp") {
                document.activeElement.scrollBy({
                    top: -100,
                    behavior: "smooth"
                })
            } else {
        ...
        if (document.activeElement.parentNode.parentElement.id == "hiraragana" || document.activeElement.parentNode.parentElement.id == "aitakata") {
            ...
            if (e.key == "ArrowUp") {
                e.preventDefault();
                setTimeout(function() {
                    document.getElementById(active).scrollBy({
                        top: -60,
                        behavior: "smooth"
                    })
                }, 50);
                if (document.activeElement.id == "focusdumbass" || document.activeElement.id == "focusdumbass1") {
                    ...
                    if (active == "hiraragana") {
                        setTimeout(function() {
                            document.querySelector("#lastitem").focus();
                        }, 600);
                    } else {
                        setTimeout(function() {
                            document.querySelector("#lastitem1").focus();
                        }, 600);
                    }
                }
            }
    ...
    if (e.key == "Enter") {
        ...
        if (document.activeElement.id == "hirBTN") {
            ...
            document.querySelector("#focusdumbass").focus();
            document.getElementById("hiraragana").scrollBy({
                top: -3000
            });
            SpatialNavigation.disable("main");
            SpatialNavigation.add({
                selector: '.hirfocusable',
                straightOnly: true,
                id: "hir",
            });
        }
        if (document.activeElement.id == "katBTN") {
            ...
            document.getElementById("aitakata").scrollBy({
                top: -3000
            });
            document.querySelector("#focusdumbass1").focus();
            SpatialNavigation.disable("main");
            SpatialNavigation.add({
                selector: '.katfocusable',
                straightOnly: true,
                id: "kat",
            });
        }
    ...
    if (e.key == "SoftRight" || e.key == "=") {
        if (document.activeElement.id == "datonebox") {
            ...
            setTimeout(function() {
                document.querySelector(".chart-pop").focus();
            }, 200);
            skorSkor();
        }
        if (document.activeElement.id == "frokus") {
...
function trollface() {
    ...
    if (getWidth() <= 240) {
        ...
        setTimeout(function() {
            document.querySelector("#trollface").style.display = "none";
        }, 2000);
    }
